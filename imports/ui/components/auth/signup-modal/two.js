...
import { createAccountAndPurchase } from '../../../../actions/flows/purchase-flow';
...
class SignupModalTwo extends Component {
  ...
  getStripeScript() {
    const { STRIPE_API_KEY } = Meteor.settings.public;

    $.getScript('https://js.stripe.com/v3/')
      .then(() => {
        // this.setState({ stripe: window.Stripe('pk_live_7Jr6qIIURxH8WTJEk7rVWtn7') });
  ...
  getErrorForField(field) {
    ...
    if (error) {
      return (
        <p className="f-response f-error-text">{error.message}</p>
      );
    }
    return (
      <noscript />
  ...
  getErrorForCard() {
    const { cardInformation: { error = '' } } = this.state;
    if (error) {
      return (
        <p className="f-response f-error-text">{error.message}</p>
      );
    ...
    return (
      <form onSubmit={e => this.handleSubmit(e)} className="dinline">
        ...
        >
          <div className="tablet-portrait">
            <div className="inner-content-container dt">
              ...
              <div className="signup-content vertical-align-center">
                ...
                <p className="title">
                  <span>Our goal is to make running traffic easier and more
                  enjoyable for you.
                  </span>
                </p>
                <div className="create-account">
                  ...
                  <div className="f-form-group">
                    <label htmlFor="signup-email-address">Email address
                      <div className="mt8 f-input-icon f-input-icon-email">
                  ...
                  <div className="f-form-group ">
                    <label htmlFor="signup-name">Full name
                      <div className="mt8 f-input-icon f-input-icon-user-circle">
                    ...
                    {this.getErrorForField('fullName')}
                  </div>

                  <div className="f-form-group">
                    <label htmlFor="signup-password">Password
                      <div className="mt8 f-input-icon f-input-icon-lock">
                  ...
                  <div className="f-form-group">
                    <span className="mb5 signup-footer-message">Card Information</span>
                    <StripeProvider stripe={this.state.stripe}>
                      <Elements>
                        <CardForm
                          ref={(c) => { this.stripeCardElement = c; }}
                        />
                      </Elements>
                    </StripeProvider>
                    {this.getErrorForCard()}
                  </div>

                  <div className="f-form-group">
                    <span className="signup-footer-message">
                      <strong>Confirmation:</strong>
                      <br />
                      Upon completing this step, we will charge your card $29.
          ...
          >
            Last step: Comment Heaven <i className="fa fa-angle-double-right" />
          </button>
        </div>
      </form>
    );
...
SignupModalTwo.propTypes = {
  ...
  createAccountAndPurchaseAction: PropTypes.func,
...
SignupModalTwo.defaultProps = {
  createAccountAndPurchaseAction: null,
  errors: [],
  user: {},
  history: {},
  closeModal: null,
...
const actions = {
  createAccountAndPurchaseAction: createAccountAndPurchase,
};
